...
class TestReceive:
    ...
    def pre_run_initialization(self):
        self.user_id = "test_receive_user_{}".format(str(uuid.uuid1()))
        self.store_id = "test_receive_store_{}".format(str(uuid.uuid1()))
        self.password = self.user_id
        self.seller = register_new_seller(self.user_id, self.password)

        self.buyer_id = "test_receive_id_{}".format(str(uuid.uuid1()))
    ...
    def test_error_order_id(self):
        for order in self.order_ids:
            code = self.buyer.receive(order + "_x")
            assert code != 200

    def test_ok(self):
        for order in self.order_ids:
            self.buyer.password = self.password
            code = self.buyer.receive(order)
            assert code == 200
