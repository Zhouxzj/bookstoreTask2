...
class TestSearchHistory:
    ...
    def pre_run_initialization(self):
        self.user_id = "test_search_history_{}".format(str(uuid.uuid1()))
        self.store_id = "test_search_history_store_{}".format(str(uuid.uuid1()))
        self.password = self.user_id
        self.seller = register_new_seller(self.user_id, self.password)

        self.buyer_id = "test_search_history_user_{}".format(str(uuid.uuid1()))
    ...
    def test_authorization_error(self):
        for order in self.order_ids:
            self.buyer.password = self.password + "_x"
